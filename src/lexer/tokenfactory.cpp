...
TokenFactory::TokenFactory(Lexer *lexer) : lexer(lexer), t() {
...
TokenFactory & TokenFactory::forEachReaden(const Checker &isFit, std::optional<Error> ifNot) {
    ...
    for (int i = t.position->index; i < lexer->reader.position.index; i++) {
        if (!isFit( lexer->reader.readCharWithOffset() )) {
...
(const function<bool(char)> &charsetHas, const Proc &ifSuccessful,
