...
namespace Charset {
    ...
    constexpr Checker AnyExceptNull     = [](char c) { return c != '\0';     };
    constexpr Checker Digit             = [](char c) { return isdigit(c)     || c == '\''; };
    constexpr Checker DigitAndDot       = [](char c) { return Digit(c)       || c == '.';  };
    ...
    constexpr Checker Punct             = [](char c) {
        return std::ranges::contains("()[];:", c);
    };;
    constexpr Checker PlusMinus         = [](char c) { return c == '+' || c == '-'; };
    constexpr Checker ExponentPrefix    = [](char c) { return c == 'e' || c == 'E'; };
};
